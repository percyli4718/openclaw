...
class ContentGenerateRequest(BaseModel):
    ...
    product_type: str = Field(
        ...,
        description="产品类型",
        json_schema_extra={"enum": ["重疾险", "医疗险", "寿险", "意外险"]}
    )
    target_audience: Optional[str] = Field(None, description="目标客户群体")
    tone: str = Field(
        default="专业",
        description="文案风格",
        json_schema_extra={"enum": ["专业", "亲和", "幽默", "紧迫"]}
    )
...
class VideoScriptRequest(BaseModel):
    ...
    duration: int = Field(default=30, description="视频时长 (秒)", json_schema_extra={"enum": [15, 30, 60]})
    style: str = Field(default="科普", description="风格", json_schema_extra={"enum": ["科普", "剧情", "访谈"]})
